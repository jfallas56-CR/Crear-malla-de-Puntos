...
try:
    ...
    _BOOL_TYPE = QMetaType.Type.Bool
...
class CrearMallaPuntos(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        param = QgsProcessingParameterNumber(
            self.HA_POR_PUNTO,
            self.tr('Hectáreas por punto (Ha/pto) [Prioridad 3: Usar si Espaciado y Densidad son 0]'),
            type=QgsProcessingParameterNumber.Double,
            defaultValue=0.0,
            optional=True,
            minValue=0.0)
    ...
    def checkParameterValues(self, parameters, context):
        ...
        _json_densidad_path = self.parameterAsFile(parameters, self.JSON_DENSIDAD_CLASES, context)  # noqa: F841
        if json_mapeo_path:
    ...
    def fill_polygon(self, polygon_geom, fields, spacing, grid_type,
        ...
        def _emit(px, py):
            nonlocal point_id, written, en_borde_count  # noqa: F824
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        try:
            ...
            if spacing_input > epsilon:
                ...
                feedback.pushInfo(
                    f"Densidad solicitada: {density_input} pts/Ha -> Espaciado calculado: {spacing:.4f} m")
                ...
                input_info_str = f"Ha/punto: {_fmt(ha_por_punto_input,
                                                   4)} ha/pto (Equiv: {_fmt(density_equiv,
                                                                            6)} pts/Ha, Dist.: {_fmt(spacing,
                                                                                                     4)} m)"
                feedback.pushInfo(
                ...
                raise QgsProcessingException(
                    "Error: Debe ingresar un valor válido en 'Espaciado', 'Densidad' o 'Ha/punto'.")
            ...
            logger.info(
                f"Iniciando algoritmo. Capa: {layer_name}, {
                    len(all_features)} polígonos, modo: {
                    self.MODE_LABELS[operation_mode]}")
            logger.info(f"Espaciado: {spacing:.4f} m, tipo malla: {self.GRID_TYPE_LABELS[grid_type]}")
            logger.info(
                f"Gestión integridad: {
                    Constants.INTEGRIDAD_NAMES[gestion_integridad]}, simplificar: {simplificar_entrada}, eliminar huecos: {eliminar_huecos}")
            ...
            if operation_mode == Constants.MODE_INDIVIDUAL:
                ...
                fields.append(QgsField("en_borde", _BOOL_TYPE))
                ...
                for i, feature in enumerate(all_features):
                    ...
                    if i % max(1, total_poligonos // 10) == 0 or i == total_poligonos - 1:
                        ...
                        logger.info(
                            f"Progreso: {i + 1}/{total_poligonos} poligonos ({_fmt(pct, 1)}%) en {elapsed:.1f}s")
                    ...
                    if densidad_clases:
                        ...
                        _clase = {k: v for k, v in densidad_clases.items() if not k.startswith('__')
                                  and k != 'meta'}.get(_id_val, {})
                ...
                if total_puntos_en_borde > 0:
                    logger.info(
                        f"[Borde] {total_puntos_en_borde} punto(s) en el perímetro del polígono (campo en_borde=True).")
                ...
                for i, feat in enumerate(all_features):
                    ...
                    if i % max(1, total_poligonos // 10) == 0 or i == total_poligonos - 1:
                        ...
                        pct = (i + 1) / total_poligonos * 100
                        logger.info(
                            f"Preparando geometrias: {i + 1}/{total_poligonos} poligonos ({_fmt(pct, 1)}%) en {elapsed:.1f}s")
            ...
            if stats['total_points'] > 0:
                ...
                stats['densidad_real'] = stats['total_points'] / \
                    stats['total_area_ha'] if stats['total_area_ha'] > 0 else 0.0
                ...
                stats['ha_por_punto_solicitado'] = ha_por_punto_input if ha_por_punto_input > epsilon else (
                    1.0 / _dens_solic if _dens_solic > 0 else 0.0)
    ...
    def _generate_report(self, params, logger, total_points, total_area_ha,
        ...
        densidad_clases_html = self._report_densidad_clases(
            params.get('densidad_clases_resumen'), params.get(
                'json_densidad_path', ''), params)
...
"""
    ...
    def _report_integrity(self, metricas, params):
        ...
        if gestion == Constants.INTEGRIDAD_REPARAR:
            consist_estado = f'[~] {geom_reparadas} reparadas con reparar geometría &nbsp;·&nbsp; [OK] {
                geom_procesadas - geom_reparadas} consistentes'
    ...
    def _report_densidad_clases(self, densidad_resumen, json_path='', params=None):
        ...
        _encabezado_rango = '<th style="padding:5px 10px;text-align:left;border-bottom:1px solid #ddd;">Ámbito</th>' if tiene_rango else ''  # noqa: E501,F841
        ...
        <div class="section">
            ...
            <div class="info-grid">
                <div class="info-card">
                    ...
                    {''.join('<div class="density-row"><span class="density-label">'
                             + self._meta_label(k)
                             + '</span>'
                             '<span class="density-value" style="font-size:12px;">'
                             + (v.replace("Jenks Natural Breaks", "Rupturas naturales (Jenks)") if isinstance(v, str) else str(v))
                             + '</span></div>' for k, v in meta.items()) if meta else ''}  
                ...
                <div class="info-card">
                    ...
                    <table style="width:100%;border-collapse:collapse;font-size:13px;">
                        <tr style="background:#f5f5f5;">
                            ...
                            <th style="padding:5px;border-bottom:1px solid #ddd;">Densidad / Espaciado</th>
                            {'<th style="padding:5px 10px;text-align:left">Intervalo</th>' if tiene_intervalo else ''}
                        </tr>{filas_html}
                    </table>
                </div>
            </div>
    ...
    def _report_mapeo(self, mapeo_resumen):
        ...
        _campo_fuente = meta.get('campo_fuente', '')  # noqa: F824,F841
        _campo_salida = meta.get('campo_salida', '')  # noqa: F824,F841
        _descripcion = meta.get('descripcion', '')  # noqa: F824,F841
        ...
        if meta_rows:
            filas = ''.join(
                f'<div class="density-row"><span class="density-label">{lbl}</span>' f'<span class="density-value" style="font-size:12px;">{
                    val.replace(
                        "Jenks Natural Breaks",
                        "Rupturas naturales (Jenks)") if isinstance(
                        val,
                        str) else val}</span></div>' for lbl,
                val in meta_rows)
